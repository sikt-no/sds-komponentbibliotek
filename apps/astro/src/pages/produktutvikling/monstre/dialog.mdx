...
{/* prettier-ignore */}
<p><Link href={`${storybookUrl}?path=/story/components-dialog--default`}>Eksempel på modal dialog som visuelt blokkerer innhold i Storybook</Link></p>
...
- Dropdown-lister for select, combobox etc.
...
{/* prettier-ignore */}
<p><Link href={`${storybookUrl}?path=/story/components-dialog--non-modal`}>Eksempel på ikke-modal dialog i Storybook</Link></p>
